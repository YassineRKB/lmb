...
class LMB_Admin_Actions_Widget extends Widget_Base {
        ...
        <div class="lmb-admin-actions-widget lmb-admin-widget">
            ...
            <div class="lmb-tab-content">
                <div id="lmb-tab-content-area">
                    <div class="lmb-loading"><i class="fas fa-spinner fa-spin"></i> Loading...</div>
                </div>
            </div>
        </div>
        <?php
    ...
    public static function get_tab_content($tab) {
        ...
        // Always get current counts for badges
        $pending_ads = get_posts(['post_type' => 'lmb_legal_ad', 'post_status' => 'any', 'posts_per_page' => -1, 'meta_query' => [['key' => 'lmb_status', 'value' => 'pending_review']]]);
        $pending_payments = get_posts(['post_type' => 'lmb_payment', 'post_status' => 'any', 'posts_per_page' => -1, 'meta_query' => [['key' => 'payment_status', 'value' => 'pending']]]);
        $pending_ads_count = count($pending_ads);
        $pending_payments_count = count($pending_payments);

        switch ($tab) {
            case 'feed':
                $content = self::render_activity_feed_content();
                break;
            case 'pending-ads':
                $content = self::render_pending_ads_content($pending_ads);
                break;
            case 'pending-payments':
                $content = self::render_pending_payments_content($pending_payments);
                break;
        }
    ...
    private static function render_pending_ads_content($pending_ads) {
        if (empty($pending_ads)) {
            return '<div class="lmb-feed-empty"><i class="fas fa-check-circle"></i><p>' . __('No legal ads are pending approval.', 'lmb-core') . '</p></div>';
        }
    ...
    private static function render_pending_payments_content($pending_payments) {
        if (empty($pending_payments)) {
            return '<div class="lmb-feed-empty"><i class="fas fa-check-circle"></i><p>' . __('No payments are pending verification.', 'lmb-core') . '</p></div>';
        }
